# T1：无头测试改走真实 proto 序列化通路

> **状态**：未开始。编号 **T1**（Test infrastructure），**不占 S 序号** —— 它不是新功能，是补一条既有测试通路的盲区。
> **执行位置**：在 S5 / S6 / S7 之前。理由见下。
> **基线**：`a2b8006a`，行号按此核对。
...
1. **它守的是叙事根基。** S5/S6/S7 都是在「位精确」这个前提上加东西，前提本身没有验证手段是倒置的。
2. **成本极低。** 改动集中在一处（见下）。
3. **S7 会立刻用到。** 击退大概率要给 `PlayerSnapshot` 加字段（加速度 / 状态位），新字段的精度问题在当前测试结构下**依然静默**。先改通路，S7 的新字段自动被覆盖。
...
**如果在 harness 里重写一份转换逻辑，这件事就白做了** —— 那验证的是 harness 自己写的那份，生产那份依然没被测到。这与 S4 决策五、S5 决策三是同一条原则：**接线层不在覆盖里，就把可判断的逻辑挪到覆盖得到的地方。**
...
放在能被 `Entity.csproj` 与测试同时引用的位置。**优先放 `GameShared/**`**（已被 `Entity.csproj:24` 通配覆盖），与 S4/S5 的分层一致。
...
| 只加用例不改通路 | 只守今天的字段，S7 加字段后盲区回归。这是本文的核心决策 |
...
见 [设计教训-两类静默失效.md](设计教训-两类静默失效.md) 末节。本阶段是「第二类：测试绕过了真实路径」的案例，与 S5 的「第一类：判据绕过了真实区间」成对讲效果更好。
