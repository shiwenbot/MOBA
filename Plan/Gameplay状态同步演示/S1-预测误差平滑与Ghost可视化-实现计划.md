...
> 与实际落地的差异：计划写四条新用例，实际五条——多一条 `error-smoothing-matching-snapshots-decay`（一致快照不得重设误差基线，否则每个快照都会重启平滑计时器）。
>
...
**与 S6 的接口约定（现在就钉死，避免将来互相踩）**：S6 要在一致路径上跳过重放。**S6 的短路判断必须放在本阶段采样代码之后**，不能放在它之前，否则会把采样一起短路掉。同理 S6 也不能整体在函数最前面 `return`（那还会连 `_authoritativeSnapshots.Save()` 一起跳掉，破坏 S3 依赖的快照历史连续性）。
...
这个值不能随便取，因为 P2 已确定加入**撞击击退**。S8 最终口径是：服务端权威裁决触发，客户端不猜触发；权威状态到达后才预测逐帧演化。因此裁决帧产生的位置修正**正是本平滑层要吸收的最大一类误差** —— 本阶段从「让回滚不那么难看」升级成了「P2 击退演示能否成立的前置条件」。
