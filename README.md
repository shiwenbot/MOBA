...
| **S5** | 无头测试改走真实 proto 序列化通路 | 半天 | **下一项**，守位精确叙事根基 |
| S6 | 回滚粒度改造：把「别人」拆出回滚 | 1.5-2 天 | 未开始，计划已写 |
| S7 | 服务端权威 RTT 测量 + 超前量控制 | 1.5 天 | 未开始，缺整条 RTT 通路 |
| S8 | Dash / 体力 / Recover / 撞击击退 | 未估 | 未开始 |
| S9 | 断线重连最小版 | 2-3 天 | 未开始 |
| S10 | 远端插值 + 轨迹线 + 完整调试面板 | 未估 | 未开始 |
| S11 | 求职展示打磨 | 未估 | 未开始 |

已完成：`P0` 方向收敛、`P1` 小球移动最小闭环、关掉 WarmStarting、带宽统计、`S1` 误差平滑、`S2` 物理层重写、`S3` 哈希上报与服务端告警、`S4` 弱网自动化测试、`B1` 属性与 Buff 增量丢包恢复、`B2` 物理体字段协议化。43 条预测自测用例。已取消：占点交互（原 `P3`，职责移交 S8 击退）、`#6` epsilon 豁免（S2 后无浮点噪声，不再需要）。
