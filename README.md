...
| **S3** | 客户端哈希上报 + 服务端告警 | 约 1 天 | **下一项**，缺服务端历史哈希索引 |
| S4 | 输入帧号 RTT 交叉校验 | 未估 | 未开始，缺整条 RTT 通路 |
...
已完成：`P0` 方向收敛、`P1` 小球移动最小闭环、关掉 WarmStarting、带宽统计、`S1` 误差平滑、`S2` 物理层重写。25 条预测自测用例。已取消：占点交互（原 `P3`）、`#6` epsilon 豁免（S2 后无浮点噪声，不再需要）。
